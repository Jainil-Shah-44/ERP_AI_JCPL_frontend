...
export default function PurchaseRequisitionListPage() {
  ...
  const load = async () => {
    const res = await getPurchaseRequisitions(1, 10000, statusFilter || undefined);
    setItems(res.data || []);
  };
  ...
  // Helper to turn "SUBMITTED" into "Submitted"
  const formatStatusForTitle = (status: string) => {
    if (!status) return "";
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex justify-between items-center">
        <h1 className="text-xl font-semibold">
          Purchase Requisition {statusFilter && `(${formatStatusForTitle(statusFilter)})`}
        </h1>
